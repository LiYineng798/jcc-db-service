...
本地验证运行 `python -m pytest -q`。新测试执行迁移中的可移植 SQL（把 PostgreSQL identity 改成 SQLite identity），校验状态约束、提交版本完整性、引用、导入和完整性检查。2026-09-14 已在 `114.134.186.41` 的临时 PostgreSQL 数据库验证全量迁移及真实 Web 封禁重审流程，随后于 15:07:33 在正式 jcc 数据库应用 0016，完整性检查全部通过；临时数据库已清理。

部署 DB 提交为 `52cc22c`，Web 为 `bd82fdd`。一致性备份位于 `/opt/jcc/deploy-backups/lineup-moderation-20260914-150418/`，包含 PostgreSQL dump、Web runtime、资料包目录、旧代码及配置；业务数据指纹在停写切换前后完全一致。Web/worker 均正常运行，19 项生产 HTTPS 检查通过。完整记录见 Web 仓库 `docs/lineup-moderation-production-deployment.md`。本条部署文档在本地提交，GitHub 推送仍由用户负责。
